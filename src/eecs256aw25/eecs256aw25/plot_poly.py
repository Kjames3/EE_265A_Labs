...
for i in range(len(waypoints)-1):
    ...
    for t in np.linspace(0, T_static, 20):
        x_r, y_r, vx_r, vy_r, ax_r, ay_r = evaluate_polynomial(alpha_x, alpha_y, t)
        v_r = math.sqrt(vx_r**2 + vy_r**2)
        theta_r = math.atan2(vy_r, vx_r)
        omega_r = (vx_r * ay_r - vy_r * ax_r) / (v_r**2) if v_r > 1e-3 else 0.0
        
        xr_list.append(x_r)
        yr_list.append(y_r)
        vr_list.append(v_r)
        omegar_list.append(omega_r)
        thetar_list.append(theta_r)

plt.figure(figsize=(12, 8))
plt.subplot(2, 2, 1)
plt.plot(xr_list, yr_list, label='Reference Traj')
plt.title("Trajectory x-y")
plt.legend()

plt.subplot(2, 2, 2)
plt.plot(vr_list, label='v_r')
plt.title("Reference Linear Vel")

plt.subplot(2, 2, 3)
plt.plot(omegar_list, label='omega_r')
plt.title("Reference Angular Vel")

plt.subplot(2, 2, 4)
plt.plot(thetar_list, label='theta_r')
plt.title("Reference Theta")

plt.tight_layout()
plt.savefig('/home/kjames/ros2_ws/src/eecs256aw25/eecs256aw25/debug_poly.png')
print("Done poly debug")
